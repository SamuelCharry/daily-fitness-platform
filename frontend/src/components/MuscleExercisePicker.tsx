...
export default function MuscleExercisePicker({ onPick }: { onPick: (ex: Exercise) => void }) {
  ...
  const exercisesForMuscle = useMemo(() => {
    ...
    return exercises
      ...
      .filter((ex) => !search.trim() || ex.name.toLowerCase().includes(search.trim().toLowerCase()));
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2, maxHeight: 260, overflowY: 'auto' }}>
        ...
        {!loading && exercisesForMuscle.length === 0 && (
          <span className="spinner-text">No exercises tagged for this muscle yet.</span>
        )}
